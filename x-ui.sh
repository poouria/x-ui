...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/poouria/x-ui/master/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
echo -e "All panel settings have been reset to default, please restart the panel now and use the default ${green}54321${plain} port to access the panel"    confirm_restart
}
...
set_port() {
    ...
    if [[ -z "${port}" ]]; then
        ...
        echo -e "The port is set，Please restart the panel now，and use the newly set port ${green}${port}${plain} 访问面板"
        confirm_restart
    fi
}
...
start() {
    ...
    if [[ $? == 0 ]]; then
        echo ""
        LOGI "The panel is already running and there is no need to start it again. If you need to restart, please select Restart"
    else
...
update_shell() {
    wget -O /usr/bin/x-ui -N --no-check-certificate https://github.com/poouria/x-ui/raw/master/x-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
...
ssl_cert_issue() {
    ...
    if [ $? -eq 0 ]; then
        ...
        if [ $? -ne 0 ]; then
            LOGE "Certificate issuance failed,script exit"
            exit 1
        else
...
show_usage() {
    ...
    echo "x-ui status       - Check x-ui state"
    echo "x-ui enable       - set up x-ui boot up"
