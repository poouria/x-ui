...
# check root
[[ $EUID -ne 0 ]] && echo -e "${red}Error:${plain} must be run as root to run this script!\n" && exit 1
...
install_x-ui() {
    ...
    if [ $# == 0 ]; then
        last_version=$(curl -Ls "https://api.github.com/repos/poouria/x-ui/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        if [[ ! -n "$last_version" ]]; then
            echo -e "${red}
Failed to detect x-ui version, it may be beyond the Github API limit, please try again later, or manually specify x-ui version installation${plain}"
            exit 1
        fi
        echo -e "Detected the latest version of x-ui:${last_version},start installation"
        wget -N --no-check-certificate -O /usr/local/x-ui-linux-${arch}.tar.gz https://github.com/poouria/x-ui/releases/download/${last_version}/x-ui-linux-${arch}.tar.gz
        if [[ $? -ne 0 ]]; then
        ...
        last_version=$1
        url="https://github.com/poouria/x-ui/releases/download/${last_version}/x-ui-linux-${arch}.tar.gz"
        echo -e "Start installing  x-ui v$1"
        wget -N --no-check-certificate -O /usr/local/x-ui-linux-${arch}.tar.gz ${url}
        if [[ $? -ne 0 ]]; then
            echo -e "${red}Download x-ui v$1 ailed, please make sure this version exists${plain}"
            exit 1
        fi
    fi
    ...
    wget --no-check-certificate -O /usr/bin/x-ui https://raw.githubusercontent.com/poouria/x-ui/main/x-ui.sh
